    ...
    public class UserController : ControllerBase
    {
        ...
        public IActionResult Authenticate(AuthenticateRequest model)
        {
            try
            {
                ...
                if (response == null)
                    return BadRequest(new { message = "User Name/Password is Incorrect / Wait Until Your Profile is Approved." });
        ...
        public IActionResult getUserDomain()
        {
            try
            {
                cmd = "select * from tbl_domain where isdeleted=0";
        ...
        public IActionResult getAllUser(int userID)
        {
            try
            {
                if (userID == 0)
                {
                    cmd = "SELECT * from view_AllUser order by firstName";    
                }
                else
                {
                    cmd = "SELECT * from view_AllUser Where userID = "+ userID +" order by firstName";
                }
        ...
        [HttpGet("getStudyLevel")]
        public IActionResult getStudyLevel()
        {
            try
            {
                cmd = "select * from tbl_studyLevel where isdeleted=0";
                var appMenu = dapperQuery.Qry<StudyLevel>(cmd, _dbCon);
                return Ok(appMenu);
            }
            catch(Exception e)
            {
        ...
        public IActionResult GoogleAuthenticate(AuthenticateRequestGoogle model)
        {
        ...
        [HttpPost("GoogleSaveUser")]
        public ActionResult GoogleSavePassenger(SaveUser model)
        {
            try
            {
                var row = dapperQuery.SPReturn("sp_savePassengerGoogle",model,_dbCon);
                return Ok(row);
            }
            catch(Exception e )
            {
        ...
        // }
        // with google 
        [HttpPost("LoginGoogleAuth")]
        public IActionResult LoginGoogleAuth(GoogleAuthenticateRequest model)
        {
            try
            {

            
            var response = _userService.GoogleLoginAuthenticate(model);
            if (response == null)
                return BadRequest(new { message = "user name or password is incorrect" });
        ...
        public IActionResult getUserInfo(int userID)
        {
            try
            {
                cmd = "select * from view_userInfo where userID="+userID+"";
                var appMenu = dapperQuery.Qry<UserInfo>(cmd, _dbCon);
                return Ok(appMenu);
            }
            catch(Exception e)
            {
        ...
        public IActionResult saveWebUser(MobileUserCreation model)
        {
            try
            {
                ...
                if (result == true && !string.IsNullOrEmpty(model.eDoc) && !string.IsNullOrEmpty(model.eDocExt))
                    {
                ...
                if (result == true && !string.IsNullOrEmpty(model.eResume) && !string.IsNullOrEmpty(model.eResumeExt))
                {
                    var companyID = data.First().res.Split("|||")[1];
                    dapperQuery.saveImageFile(
                        model.eResumePath,
        ...
        [HttpPost("saveOriginalCredential")]
        public IActionResult saveOriginalCredential(OriginalCredentailCreation model)
        {
            try
            {
                var response = dapperQuery.SPReturn("sp_changeOriganalCredential",model,_dbCon);
        ...
        [HttpGet("getPartyID")]
        public IActionResult getPartyID(int userID)
        {
            try
            {
                cmd = "select distinct * from view_getPartyID where userID="+userID+"";
                var appMenu = dapperQuery.Qry<GetParty>(cmd, _dbCon);
                return Ok(appMenu);
            }
            catch(Exception e)
            {
